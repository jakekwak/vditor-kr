...
export const formatRender2 = (vditor: IVditor, content: string, position?: { start: number, end: number }, options = {
    ...
    if (textList.length <= 2 && isEmpty) {
        ...
        vditor.sv2.element.innerHTML = "";
    } else {
        // TODO: 使用虚拟 Dom
        vditor.sv2.element.innerHTML = html || newLine;
    }

    if (position) {
        setSelectionByPosition(position.start, position.end, vditor.sv2.element);
    }
