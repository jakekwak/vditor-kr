...
export const processKeydown2 = (vditor: IVditor, event: KeyboardEvent) => {
    vditor.undo.recordFirstPosition(vditor, event);

    const editorElement = vditor.sv2.element;
    ...
    if (vditor.options.tab && event.key === "Tab") {
        ...
        if (event.shiftKey) {
            ...
            formatRender2(vditor, text.substring(0, selectLinePosition.start) +
        ...
        if (position.start === position.end) {
            insertText2(vditor, vditor.options.tab, "");
            return true;
        }
        const selectionResult = selectLineList.map((value) => {
            return vditor.options.tab + value;
        }).join("\n");
        formatRender2(vditor, text.substring(0, selectLinePosition.start) + selectionResult +
    ...
    //     formatRender2(vditor, deletedText, {
    //         end: startIndex,
    //         start: startIndex,
    //     });
    //     event.preventDefault();
